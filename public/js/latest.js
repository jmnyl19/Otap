$(document).ready(function () {
  ...
  function generatePaginationLinks(pagination) {
    ...
    html += '<li class="page-item ' + (pagination.current_page === 1 ? 'disabled' : '') + '"><a class="page-link" href="#" onclick="getReport(' + (pagination.current_page - 1) + ')">Previous</a></li>';
    for (var i = 1; i <= pagination.last_page; i++) {
        html += '<li class="page-item ' + (i === pagination.current_page ? 'active' : '') + '"><a class="page-link" href="#" onclick="getReport(' + i + ')">' + i + '</a></li>';
    }
    html += '<li class="page-item ' + (pagination.current_page === pagination.last_page ? 'disabled' : '') + '"><a class="page-link" href="#" onclick="getReport(' + (pagination.current_page + 1) + ')">Next</a></li>';
    html += '</ul>';
    return html;
}
  function getReport(page) {
    ...
    $.ajax({
      url: '/getreport?page=' + page,
      type: 'GET',
      dataType: 'json',
      success: function (response) {
        $('#getReportsCont').empty(); 
  ...
  function reportModal(id) {
    ...
    $.ajax({
        ...
        success: function (response) {
          ...
          if (response.history8[0].Ambulance == 1){
            var aStatus = "ambulanceActive"
          }

          if (response.history8[0].Firetruck == 1){
            var fStatus = "firetruckActive"
          }

          if (response.history8[0].BPSO == 1){
          ...
          <div class="col-12 mt-3">
              ...
              <textarea class="form-control border-2 border-dark-subtle align-left" id="exampleFormControlTextarea1"  name="ticket_body" rows="3" aria-label="Disabled input example" disabled readonly>${response.history8[0].type_of_incidents}</textarea>
          </div>
          <div class="col-12 mt-3">
          ...
          var pendingFooter = `
              ...
              <form action="" method="POST">
                  <button class="forwardBtn" type="button" onclick="unavailablereport(${response.history8[0].id})">
