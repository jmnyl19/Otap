$(document).ready(function () {
    getCancelledReport(1);
  });
  function generatePaginationLinks(pagination) {
    var html = '<ul class="pagination">';
    html += '<li class="page-item ' + (pagination.current_page === 1 ? 'disabled' : '') + '"><a class="page-link" href="#" onclick="getCancelledReport(' + (pagination.current_page - 1) + ')">Previous</a></li>';
    for (var i = 1; i <= pagination.last_page; i++) {
        html += '<li class="page-item ' + (i === pagination.current_page ? 'active' : '') + '"><a class="page-link" href="#" onclick="getCancelledReport(' + i + ')">' + i + '</a></li>';
    }
    html += '<li class="page-item ' + (pagination.current_page === pagination.last_page ? 'disabled' : '') + '"><a class="page-link" href="#" onclick="getCancelledReport(' + (pagination.current_page + 1) + ')">Next</a></li>';
    html += '</ul>';
    return html;
}
  function getCancelledReport(page) {
    $.ajaxSetup({
      headers: {
    ...
    $.ajax({
      url: '/getcancelledreport?page=' + page,
      type: 'GET',
      dataType: 'json',
      success: function (response) {
        $('#cancelledReports').empty(); 
        if (response.canpreport.length === 0) {
            var incidentHtml = '';
                ...
                <div class="btn btn-primary shadow p-4 mb-1 bg-white rounded" type="button" style="width: 100%; border: none">
                    <div class="card-body">
                        <div class="row align-items-center text-start">
                            ...
                            <div class="col">
                                <h6 style="color: #ababab; text-align: center;" ><i>No cancelled incidents!</i><span class="fw-bold"></span></h6>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            $('#cancelledReports').append(incidentHtml);
          } else {
        $.each(response.canpreport, function(index, value) {
  ...
  function comReportsModal(id) {
    ...
    $.ajax({
        ...
        success: function (response) {
          ...
          $('#cancelledReportsModalBody').empty();
          $('#cancelledReportsModal').modal('show');
          var incidentHtml = `
          ...
          `;
          $('#cancelledReportsModalBody').append(incidentHtml);
  ...
  function exportExcel(page) {
    $.ajax({
        ...
        success: function (response) {
            ...
            generateExcel(response.canpreport);
        },
        error: function (error) {
