    ...
    $.ajax({
      ...
      success: function (response) {
        ...
        if (response.unavailablereport.length === 0) {
            ...
            incidentHtml += `
            <div class="btn btn-primary shadow p-4 mb-1 bg-white rounded" type="button" style="width: 100%; margin: 10px; border: none">
            <div class="card-body">
                <div class="row align-items-center text-start">
                    <div class="col-auto">
                    </div>
                    <div class="col">
                        <h6 style="color: #ababab; text-align: center;" ><i>No unavailable incidents!</i><span class="fw-bold"></span></h6>
                    </div>
                </div>
            </div>
        </div>
        ...
        $.each(response.unavailablereport, function(index, value) {
  ...
  function reportsForModal1(id) {
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            if (response.history13[0].Ambulance == 1){
              var aStatus = "ambulanceActive"
            }
  
            if (response.history13[0].Firetruck == 1){
              var fStatus = "firetruckActive"
            }
  
            if (response.history13[0].BPSO == 1){
              var bStatus = "bpsoActive"
            }

            var imagePath = 'file/' + response.history13[0].file;
          ...
          <div class="col-md-6 mt-3">
              ...
              <input type="text" class="form-control border-2 border-dark-subtle" id="inputName" name="id" value="${response.history13[0].user.first_name} ${response.history13[0].user.last_name}" aria-label="Disabled input example" disabled readonly>
          </div>
          <div class="col-md-3 mt-3">
              <label for="inputPhone" class="form-label mb-0 fs-6">Phone Number</label>
              <input type="text" class="form-control border-2 border-dark-subtle" id="inputPhone" name="id" value="${response.history13[0].user.contact_no}" aria-label="Disabled input example" disabled readonly>
          </div>
          <div class="col-md-3 mt-3">
              <label for="inputPassword4" class="form-label mb-0 fs-6">Date</label>
              <input type="text" class="form-control border-2 border-dark-subtle" id="inputPassword4" name="id" value="${response.history13[0].datehappened}" aria-label="Disabled input example" disabled readonly>
          </div>
          <div class="col-12 mt-3">
            ...
            <div class="col-12 mt-3">
              ...
              <textarea class="form-control border-2 border-dark-subtle align-left" id="exampleFormControlTextarea1"  name="ticket_body" rows="3" aria-label="Disabled input example" disabled readonly>${response.history13[0].type_of_incidents}</textarea>
          </div>
          <div class="col-12 mt-3">
              <label for="exampleFormControlTextarea1" class="form-label">Incident Details</label>
              <textarea class="form-control border-2 border-dark-subtle align-left" id="exampleFormControlTextarea1"  name="ticket_body" rows="3" aria-label="Disabled input example" disabled readonly>${response.history13[0].details}</textarea>
          </div>
          <div class="col-12 mt-3 mb-3">
              <label for="inputAddress" class="form-label mb-0">Additional Notes</label>
              <input type="text" class="form-control border-2 border-dark-subtle" id="inputPassword4" name="id" value="${response.history13[0].addnotes}" aria-label="Disabled input example" disabled readonly>
          </div>
          ...
          $('#unavailablereportFooter').empty();
          var unavailablereport = `

          <form action="" method="POST">
          <button class="respondBtn" type="button" onclick="responding(${response.history13[0].id})">
          ...
          $('#unavailablereportFooter').append(unavailablereport);
